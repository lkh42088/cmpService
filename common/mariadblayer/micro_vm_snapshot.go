...
func (db *DBORM) GetMcVmSnapshotPageParam(paging models.Pagination, cpIdx string, serverIdx string, name string) (obj mcmodel.McVmSnapPage,
	err error) {
	var query string

	query = "snap_cp_idx = '" + cpIdx + "' and snap_vm_name = '" + name + "' and snap_server_idx" +
		" = '" + serverIdx + "'"

	err = db.Table("mc_vm_snapshot_tb").Debug().
		Select("mc_vm_snapshot_tb.*, c.cp_name, m.mc_serial_number").
		Joins("INNER JOIN company_tb c ON c.cp_idx = mc_vm_snapshot_tb.snap_cp_idx").
		Joins("INNER JOIN mc_server_tb m ON m.mc_idx = mc_vm_snapshot_tb.snap_server_idx").
		Order(obj.GetOrderBy(paging.OrderBy, paging.Order)).
	...
	db.Table("mc_vm_snapshot_tb").Debug().
		Select("mc_vm_snapshot_tb.*, c.cp_name, m.mc_serial_number").
		Joins("INNER JOIN company_tb c ON c.cp_idx = mc_vm_snapshot_tb.snap_cp_idx").
		Where(query).
		Count(&paging.TotalCount)
