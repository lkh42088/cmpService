...
type IpMgmt struct {
	Idx				int			`gorm:"primary_key;type:int(11);unsigned;auto_increment;column:'idx';comment:'INDEX'"`
	DeviceCode		string		`gorm:"type:varchar(12);not null;column:'device_code';comment:'장비 코드'"`
	DevTag			string		`gorm:"type:varchar(50);not null;column:'dev_tag';comment:'DEVICE TAG'"`
	...
	NatIp			string		`gorm:"type:varchar(15);not null;column:'nat_ip';comment:'NAT IP'"`
	DevDesc			string		`gorm:"type:varchar(30);not null;column:'ip_pdesc';comment:'DEVICE 상세'"`
	PortDesc		string		`gorm:"type:varchar(30);not null;column:'ip_port_desc';comment:'PORT 상세'"`
	DevMemo			string		`gorm:"type:text;not null;column:'ip_memo';comment:'IP MEMO'"`
	...
	DevUse			bool		`gorm:"type:tinyint;not null;default:1;column:'ip_use';comment:'사용 여부'"`
}
...
type SubnetMgmt struct {
	Idx				int			`gorm:"primary_key;type:int(11);unsigned;auto_increment;column:'idx';comment:'INDEX'"`
	DeviceCode		string		`gorm:"type:varchar(12);not null;column:'device_code';comment:'장비 코드'"`
	NetworkId		int			`gorm:"type:int(11);not null;column:'sub_net_id_cd';comment:'네트워크 코드'"`
