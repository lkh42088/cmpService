...
func RunAPI(address string, db *mariadblayer.DBORM) error {
	...
	router.GET(lib.SvcmgrApiMicroVmStats+"/:mac", GetBaremetalInterfaceTrafficByMac)
	...
	// keepalive, registration from mcagent
	router.POST(lib.SvcmgrApiMicroServerRegularMsg, h.ReceiveRegularMsg)
	...
	router.GET(lib.SvcmgrApiMicroTotalCount, h.GetTotalCount)
	router.GET(lib.SvcmgrApiMicroVmCount+"/:cpName", h.GetVmTotalCountByCpName)
	...
	router.POST(lib.SvcmgrApiMicroMcAgentNotifyBackup, h.StoreVmBackupFromMc)
	...
	// System Action
	router.POST(lib.SvcmgrApiSystemModifyConf, ModifyConfVariable)
	router.POST(lib.SvcmgrApiSystemRestartAgent, RestartAgent)
