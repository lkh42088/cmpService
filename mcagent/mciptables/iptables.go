...
func NewWithProtocol(proto Protocol) (*IPTables, error) {
	//fmt.Println("NewWithProtocol: proto", proto)
	path, err := exec.LookPath(getIptablesCommand(proto))
	if err != nil {
		return nil, err
	}
	//fmt.Println("NewWithProtocol: path", path)
	vstring, err := getIptablesVersionString(path)
	if err != nil {
		return nil, fmt.Errorf("could not get iptables version: %v", err)
	}
	//fmt.Println("NewWithProtocol: version", vstring)
	v1, v2, v3, mode, err := extractIptablesVersion(vstring)
	if err != nil {
		return nil, fmt.Errorf("failed to extract iptables version from [%s]: %v", vstring, err)
	}
	//fmt.Println("NewWithProtocol: v", v1, v2, v3, mode)

	checkPresent, waitPresent, randomFullyPresent := getIptablesCommandSupport(v1, v2, v3)
	//fmt.Println("NewWithProtocol: support", checkPresent, waitPresent, randomFullyPresent)
...
func (ipt *IPTables) executeList(args []string) ([]string, error) {
	...
	//fmt.Println("executeList:",stdout.String())
